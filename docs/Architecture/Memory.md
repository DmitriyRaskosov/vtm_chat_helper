...
Мосты к канону (явные, не из текста): `memory_node_entities` (роль сущности), `memory_node_messages`, `memory_node_events`, `memory_node_lore_entries`, `memory_node_scenes`. `MemoryBridgeService` не копирует `node_text` в лор/сущность. Для NPC lore-мост виден только если статья доступна по допуску/исключениям; `storyteller_only` события скрыты.
...
Видимость UI лора (`public` / `storyteller_only`) не равна знанию NPC: знание даёт членство `classification` в `lore_clearance_levels`, флаг `situational` и исключения `character_lore_knowledge`. Правила по-прежнему через `character_rule_knowledge`. Собственная биография доступна персонажу без гранта лора.

См. [[Architecture/Lore]], [[Architecture/Retrieval]], [[Architecture/Backend]], [[Archive/Architecture Migration]].
