...
**Статус:** этапы 1–4 в коде (этап 4: 2026-09-14). Текущая работа хроники — [[Project/ClanCard]], не этот файл. Этап 1: драйвер Ollama, `extraction_runs`, POST/GET `/api/extract`, PHP-матчинг. Этап 2: accept/discard, кнопка «Разобрать» на статье лора. Этап 3: био → память. Этап 4: сцена → events + relations, тонкий HTTP событий. **После:** память сцены (кто что запомнил) — отдельно; парсер профиля клана — секция ниже, после ClanCard. Заменяет отложенный «этап 3» прицела (LLM-черновик узлов/рёбер на accept). Не этап архива [[Archive/Architecture Migration]].

Канон экстрактора — эта заметка. Живой пошаговый план продукта сейчас — [[Project/ClanCard]]. Файл в `.cursor/plans/` каноном не является.
...
Длинная статья лора (&gt; 10k символов): непересекающиеся char-окна по 10k; один POST = одно окно; `extraction_runs.from_char_offset` / `to_char_offset`; курсор — max `to_char_offset` у `reviewed` runs. После полного прохода обычный POST блокируется; `reparse: true` (или кнопка «Разобрать заново») supersede-ит все прогоны статьи и начинает с offset 0. Inbox: `POST /api/extract/{run}/reparse` для одного char-окна лора.
...
| Традиции, Маскарад, кодексы, статьи кодекса | `concept` | `part_of`: source = часть (статья), target = целое (кодекс, традиция); **не** `member_of` и **не** `created` от фракции |
...
## После: профиль клана (backlog, не этот цикл)

Не раздувать текущий лор-промпт и не писать код парсера, пока [[Project/ClanCard]] не в каноне. Тогда — отдельный JSON-профиль лора (не общий со сценой):

- Кандидаты в те же сервисы, что UI: поля `clans`, sync `clan_disciplines` по каталогу (русские имена → `key`), `regards`+`note`.
- «Отступники X» как цель стереотипа → matched aka X, не `new_entity`.
- Сцена: события и отношения персонажей, не новые дисциплины и не карточки кланов.
- Один system prompt на профиль+стереотипы+дисциплины+`part_of` для 8B не планировать.
